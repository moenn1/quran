...
2. Review `.env.api.example`, `.env.web.example`, and `.env.cli.example` if you want service-specific overrides.
3. Leave `COMPOSE_PROFILES=` empty for the default stack, or set `COMPOSE_PROFILES=semantic-search` in `.env` if you want the optional Qdrant service.
4. Run `docker compose up --build`.
5. Open `http://localhost:3000` for the bootstrap web surface.
6. Call `http://localhost:8000/health` for the bootstrap API health check.
7. Run `./scripts/smoke-compose.sh` when you want a scripted self-hosting verification pass. The smoke test uses an isolated Compose project and high host ports so it does not interfere with a normal local stack.
...
- `.env.example`: shared Compose defaults for ports, database credentials, privacy mode, and Compose profiles such as `COMPOSE_PROFILES=semantic-search`.
...
- `.env.cli.example`: CLI-specific defaults for API base URL, local data directory, privacy mode, and semantic-search wording. The CLI is not containerized in the bootstrap stack yet, but its privacy and wording contract is documented here now.
...
- The current bootstrap stack does not include a CLI container.
