...
4. Review `.env.api.example`, `.env.web.example`, and `.env.cli.example` for service-specific overrides.
5. Create a local Python virtual environment when running repository or data-validation tooling:
   `python3 -m venv .venv`
   `. .venv/bin/activate`
6. Install JavaScript workspace dependencies with `npm ci` when frontend code is present.
7. Start the bootstrap stack with `docker compose up --build`.
8. Run `./scripts/release-readiness.sh` before pushing changes that affect repository quality, docs, Docker, CI, or data validation.
